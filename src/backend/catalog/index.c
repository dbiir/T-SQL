...
static double IndexBuildHeapScan(Relation heapRelation,
								 ...
								 void *callback_state);
static double IndexBuildRocksDBScan(Relation heapRelation,
								 Relation indexRelation,
								 struct IndexInfo *indexInfo,
								 bool allow_sync,
								 EState *estate,
...
kvsecondindex_build(Relation heapRelation,
					Relation indexRelation)
{
	IndexBuildResult *stats = palloc0(sizeof(*stats));
	stats->heap_tuples = heapRelation->rd_rel->reltuples;
	stats->index_tuples = 0;
	/* init a kv store space */
	TupleTableSlot* slot = MakeSingleTupleTableSlot(RelationGetDescr(heapRelation));
	TransactionId xid = GetCurrentTransactionId();
	/* init the kv build desc */
	Oid pk_oid = get_pk_oid(heapRelation);
	Assert(OidIsValid(pk_oid));
	int pk_natts = 0;
	int index_natts = indexRelation->rd_index->indnatts;
	int *pk_colnos = get_index_colnos(pk_oid, &pk_natts);

	Size size = mul_size(sizeof(int), indexRelation->rd_index->indnatts);
	int *index_colnos = palloc0(size);
	for (int i = 0; i < indexRelation->rd_index->indnatts; i++)
	{
		index_colnos[i] = (int)indexRelation->rd_index->indkey.values[i];
	}
	...
	initkey.rel_id = heapRelation->rd_id;
	initkey.xid = xid;
	initkey.isend = false;
	initkey.cid = 0;

	KVEngineScanDesc scan = kvengine_beginscan(heapRelation, NULL);
	while (true)
	{
		...
		init_pk_in_keydesc(&initkey, pk_oid,
						get_attr_type(heapRelation, pk_colnos, pk_natts),
						pkvalues, pkisnull, pk_natts);
		init_sk_in_keydesc(&initkey, indexRelation->rd_id,
						get_attr_type(heapRelation, index_colnos, index_natts),
						skvalues, skisnull, index_natts);
		TupleKeySlice secondkey = build_key(initkey);

		/* make value as pk value */
		TupleValue kvengine_value = (TupleValue)palloc0(sizeof(*kvengine_value));
		...
		TupleValueSlice secondvalue = {kvengine_value, sizeof(*kvengine_value)};

		kvengine_check_unique_and_insert(heapRelation, secondkey, secondvalue,
										slot->tts_rangeid, true);
	...
	pfree(pk_colnos);
	pfree(index_colnos);
	kvengine_endscan(scan);
	return stats;
}

static IndexBuildResult*
kvindex_build(Relation heapRelation,
			Relation indexRelation,
			bool isprimary)
{
	if (isprimary)
	{
		IndexBuildResult *stats = palloc0(sizeof(*stats));
		stats->heap_tuples = heapRelation->rd_rel->reltuples;
		stats->index_tuples = heapRelation->rd_rel->reltuples;
		return stats;
	}
	else
	{
		return kvsecondindex_build(heapRelation, indexRelation);
	}
}
/*
			...
			bool isreindex)
{
	 ...
	 */
	if (RelationIsRocksDB(heapRelation))
	{
		stats = kvindex_build(heapRelation, indexRelation, isprimary);
	}
			   ...
			   void *callback_state)
{
	...
	if (RelationIsHeap(parentRelation))
		...
		reltuples = IndexBuildAppendOnlyColScan(parentRelation,
												...
												callback,
												callback_state);
	else if (RelationIsRocksDB(parentRelation))
		reltuples = IndexBuildRocksDBScan(parentRelation,
									   indexRelation,
									   indexInfo,
									   allow_sync,
									   estate,
...
static double
IndexBuildRocksDBScan(Relation heapRelation,
				   Relation indexRelation,
				   struct IndexInfo *indexInfo,
				   bool allow_sync,
				   EState *estate,
				   ...
				   void *callback_state)
{
	...
	TupleTableSlot *slot;
	List *predicate = NIL;
	Assert(RelationIsRocksDB(heapRelation));
	Assert(estate->es_per_tuple_exprcontext != NULL);
	
	/* Remember if it's a system catalog */
	/* 目前应该不可能出现ROCKSDB有系统表的情况 */
	is_system_catalog = IsSystemRelation(heapRelation);
	...
	predicate = (List *)
		ExecPrepareExpr((Expr *) indexInfo->ii_Predicate,
						estate);
	/* 首先进行全表扫描 */
	KVEngineScanDesc scandesc = kvengine_beginscan(heapRelation, snapshot);
	do
	{
		kvengine_getnext(scandesc, ForwardScanDirection, slot);
		CHECK_FOR_INTERRUPTS();
		/* TODO:下来实际上还需要判断是否存在对该条记录进行插入或删除的情况 */
		...
		/* 这里我目前直接插入了，是有问题的。 */
		kv_index_insert(indexRelation, slot, 0, heapRelation, checking_uniqueness);
		
	} while (!TupIsNull(slot));
	
	/* 然后获取到所有kv以后，进行判断，包括唯一性判断和其他约束 */
	return 0;
}
