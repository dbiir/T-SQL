...
/*
 * RelationCreateRocksDBStorage
 *		Create physical storage for a rocksdb relation.
 *
 * Create both the underlying disk file storage for the relation and
 * the metadata entries for the relation in rocksdb. For the underlying
 * disk file, this only creates the main fork; additional forks are 
 * created lazily by the modules that need them.
 *
 * This function calls RelationCreateStorage, which is transactional
 * and WAL-logged, so that if the transaction aborts later on, the 
 * storage (both disk file and rocksdb metadata entries) will be 
 * destroyed.
 */
void
RelationCreateRocksDBStorage(Relation rel, char relpersistence, char relstorage)
{
	/* TODO: create the metadata entries of this relation in rocksdb */
...
RelationDropStorage(Relation rel)
{
	...
	/**
	 * If this is a rocksdb relation, also remove the kv entries of this ralation
	 * from rocksdb.
	 */
	if (rel->rd_rel->relstorage == RELSTORAGE_ROCKSDB)
	{
		/* TODO: delete the kv entries of this relation in rocksdb */
	}
