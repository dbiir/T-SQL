			...
			bool allow_system_table_mods)
{
	...
	{
		bool isAppendOnly;
		bool isRocksDB; /* if rocksdb storage engine is used. */
		...
		isRocksDB = (relstorage == RELSTORAGE_ROCKSDB);

		RelationOpenSmgr(rel);
		
		/**
		 * If this is a rocksdb relation, also create the column family for it
		 * in rocksdb.
		 */
		if (isRocksDB)
		{
			RelationCreateRocksDBStorage(rel, relpersistence, relstorage);
		}
		else
		{
		 ...
		 * smgr open for it.
		 * Rocksdb tables also do not use buffer manager.
		 */
		if (isAppendOnly || isRocksDB)
						 ...
						 bool is_part_parent)
{
	...
	bool		appendOnlyRel;
	bool		rocksdbRel; /* is true if storage_engine=rocksdb */
	...
	if (relkind == RELKIND_RELATION)
	{
		...
		rocksdbRel = (pg_strcasecmp(stdRdOptions->storage_engine, ROCKSDB_STORAGE_ENGINE) == 0);
		/* Firstly check if this is a rocksdb relation */
		if (rocksdbRel)
		{
			relstorage = RELSTORAGE_ROCKSDB;
		}
		else if(appendOnlyRel)
		{
	...
	{
		/** 
		 * Must be set to false here, otherwise they do not have determined 
		 * values, which may cause crash when initializing database.
		 */
		appendOnlyRel = false;
		rocksdbRel = false;
		stdRdOptions = NULL;
	...
	/*
	 * if this is a rocksdb relation, add an entry in pg_rocksdb.
	 */
	if (rocksdbRel)
	{
		InsertRocksDBEntry(relid, relname, stdRdOptions->cf_options, NIL);
	}
	/*
	 * else if this is an append-only relation, add an entry in pg_appendonly.
	 */
	else if(appendOnlyRel)
	{
...
heap_drop_with_catalog(Oid relid)
{
	...
	is_rocksdb_rel = RelationIsRocksDB(rel);
