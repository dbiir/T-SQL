...
static bool
_equalTemporal(const Temporal *a, const Temporal *b)
{
	COMPARE_STRING_FIELD(lower);
	COMPARE_STRING_FIELD(upper);
...
static bool
_equalRangePlan(const RangePlan a, const RangePlan b)
{
	if (a->plan_id != b->plan_id)
		return false;
	if (a->ms_plan_type != b->ms_plan_type)
		return false;
	return true;
}

static bool
_equalMQHandlePlan(const KVEngineMQHandle a, const KVEngineMQHandle b)
{
	if (a->pid != b->pid)
		return false;
	return true;
...
equal(const void *a, const void *b)
{
	...
	switch (nodeTag(a))
	{
		...
		case T_DistributedBy:
			...
			break;
		case T_RangePlan:
			retval = _equalRangePlan((RangePlan)a, (RangePlan)b);
            break;
		case T_MQ_HandlePlan:
			retval = _equalMQHandlePlan((KVEngineMQHandle)a, (KVEngineMQHandle)b);
            break;
		default:
