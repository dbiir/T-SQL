...
_readCreateStmt_common(CreateStmt *local_node)
{
	...
	READ_OID_FIELD(relRangeID);	
	READ_INT_FIELD(replicanum);
	int replica_num = local_node->replicanum;
	READ_INT_ARRAY(replica_segid, replica_num, int32);
...
_readMotion(void)
{
	...
	Assert(local_node->motionType == MOTIONTYPE_FIXED || 
			local_node->motionType == MOTIONTYPE_HASH || 
			local_node->motionType == MOTIONTYPE_EXPLICIT ||
			local_node->motionType == MOTIONTYPE_RANGE);
...
_readSlice(void)
{
	...
	READ_OID_FIELD(directDispatch.rangeid);
