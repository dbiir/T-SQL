...
static BackgroundWorker PMAuxProcList[MaxPMAuxProc] =
{
	{"ftsprobe process",
	 ...
	 FtsProbeStartRule},
#ifndef DISABLE_RANGE_DISTRIBUTED
	{"msprobe process",
     BGWORKER_SHMEM_ACCESS | BGWORKER_BACKEND_DATABASE_CONNECTION,
     BgWorkerStart_DtxRecovering,
     0, /* restart immediately if msprobe exits with non-zero code */
     MSMain, {0}, {0}, 0, 0,
	 MSStartRule},
#endif
	{"storage process",
	...
	if (PG_PROTOCOL_MAJOR(proto) >= 3)
	{
		...
		while (offset < len)
		{
			...
			else if (strcmp(nameptr, GPCONN_TYPE) == 0)
			{
				...
				else if (strcmp(valptr, GPCONN_TYPE_MS) == 0)
				{
					am_mshandler = true;
				}
#ifdef FAULT_INJECTOR
...
#ifdef FAULT_INJECTOR
	if (!am_ftshandler && !IsFaultHandler && !am_mshandler && !am_walsender &&
...
BackendInitialize(Port *port)
{
	...
	if (am_walsender)
		...
		init_ps_display("ms handler process", port->user_name, remote_ps_data,
						update_process_title ? "authentication" : "");
