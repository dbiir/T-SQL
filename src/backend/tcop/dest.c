...
EndCommand(const char *commandTag, CommandDest dest)
{
	switch (dest)
	{
		...
		case DestRemoteExecute:
        {
            ...
            {
                global_tmp_timestamp.lts_low = CurrentReadWriteSetSlot->lower;
                global_tmp_timestamp.lts_high = CurrentReadWriteSetSlot->upper;
            }

            pq_sendint64(&buf, global_tmp_timestamp.lts_low);
            pq_sendint64(&buf, global_tmp_timestamp.lts_high);
            pq_sendint64(&buf, (int64)global_tmp_timestamp.start_ts);
            pq_endmessage(&buf);
