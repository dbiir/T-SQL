...
{
	if (am_ftshandler || am_mshandler || IsFaultHandler)
	{
		switch (firstchar)
		{
			case 'Q':
			...
			default:
				...
				ereport(ERROR,
						(errcode(ERRCODE_PROTOCOL_VIOLATION),
						 errmsg("protocol '%c' is not supported in a MS connection",
								firstchar)));
		}
	}
}
			 ...
			 const char *username)
{
	...
	{
		Oid dbid = GpIdentity.dbid;
		gp_session_id = gp_session_id * 10 + (int)dbid;
	}
	/*
	...
	if (!(am_ftshandler || IsFaultHandler || am_mshandler) && Gp_role == GP_ROLE_EXECUTE)
	{
	...
	MessageContext = AllocSetContextCreate(TopMemoryContext,
   	...
   	/* [williamcliu] Init storage mq. */
    if (IsUnderPostmaster && !am_startup && !am_walsender && whereToSendOutput == DestRemote)
    {
		SessionInitMQ();
        ConnectToLts();
    }
			...
			case 'I':			/* MS message */
				{
					...
					/* Set statement_timestamp() */
					SetCurrentStatementStartTimestamp();
					int length = (int)pq_getmsgint(&input_message, 4);
					query_string = pq_getmsgbytes(&input_message, length);
					pq_getmsgend(&input_message);
            ...
            case 'M': /* MPP dispatched stmt from QD */
				{
                    ...
                    const char *serializedDtxTS = NULL;
                    ...
                    int serializedDtxTSlen = DtxTimeStamp_SerializeSize();
                    ...
                    serializedDtxTSlen = pq_getmsgint(&input_message, 4);

                    if (serializedDtxTSlen != 0)
                    {
                        serializedDtxTS = pq_getmsgbytes(&input_message, serializedDtxTSlen);

                        DtxTimeStamp_Deserialize(serializedDtxTS, serializedDtxTSlen, &global_tmp_timestamp);
                        if (CurrentReadWriteSetSlot)
                        {
                            if (!CheckLogicalTsIntervalValid(global_tmp_timestamp.lts_low, global_tmp_timestamp.lts_high))
							{
								// TODO: not valid timestamp range, throw an error in log, but we will not abort here.
								// abort in master
							}
                        }
                    }
            ...
            case 'T': /* MPP dispatched dtx protocol command from QD */
				{
                    ...
                    if (serializedDtxTSlen != 0)
                    {
                        ...
                        if (CurrentReadWriteSetSlot)
                        {
                            if (!CheckLogicalTsIntervalValid(global_tmp_timestamp.lts_low, global_tmp_timestamp.lts_high))
							{
								// TODO: same as the below code and abort in master
							}
                        }
                    }
