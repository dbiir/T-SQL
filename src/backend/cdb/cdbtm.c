...
getDistributedTransactionId(void)
{
	...
	else if (isQEContext() || DistributedTransactionContext == DTX_CONTEXT_QE_FINISH_PREPARED)
	{
...
notifyCommittedDtxTransaction(void)
{
	...
	if (transam_mode == TRANSAM_MODE_NEW_OCC)
		global_tmp_timestamp.commit_ts = CurrentReadWriteSetSlot->lower;
	else 
		global_tmp_timestamp.commit_ts = GetTimeStamp();
...
tmShmemInit(void)
{
	...
	if (!found)
	{
		...
		*shmGIDSeq = GpIdentity.dbid/* == 1? 2 : GpIdentity.dbid % 2*/;
...
generateGID(void)
{
	...
	gxid = pg_atomic_add_fetch_u32((pg_atomic_uint32*)shmGIDSeq, PENGUINDB_GLOBAL_MASTER_NUMBER);
...
performDtxProtocolPrepare(const char *gid)
{
	 ...
	 */
	if (Gp_role == GP_ROLE_EXECUTE
		&& transam_mode == TRANSAM_MODE_NEW_OCC)
	{
		bool success = kvengine_prepare();
		if (!success)
		{
			elog(ERROR, "DTA Prepare of distributed transaction %s failed", gid);
			return;
		}
	}
...
performDtxProtocolCommitPrepared(const char *gid, bool raiseErrorIfNotFound)
{
	...
	if (Gp_role == GP_ROLE_EXECUTE)
	{
		if (transam_mode != TRANSAM_MODE_DEFAULT
			|| transaction_type == KVENGINE_TRANSACTIONDB)
		{
			kvengine_commit();
		}
	}
...
performDtxProtocolAbortPrepared(const char *gid, bool raiseErrorIfNotFound)
{
	...
	// Writebatch to db
	if (Gp_role == GP_ROLE_EXECUTE
		&& (transam_mode != TRANSAM_MODE_DEFAULT
			|| transaction_type == KVENGINE_TRANSACTIONDB))
	{
		kvengine_abort();
		// TODO: clean read txn set
	}
