...
reconstructTuple(MotionNodeEntry *pMNEntry, ChunkSorterEntry *pCSEntry, TupleRemapper *remapper)
{
	 ...
	 */
	tup = CvtChunksToTup(&pCSEntry->chunk_list, pSerInfo, remapper, &key, &rangeid);
	...
	htfifo_addtuple(pCSEntry->ready_tuples, tup, key, rangeid);
		  ...
		  int16 targetRoute)
{
	...
	sent = SerializeTuple(slot, key, rangeid, &pMNEntry->ser_tup_info, &b, &tcList, targetRoute);
			  ...
			  int16 srcRoute)
{
	...
	for (;;)
	{
		...
		tuple = htfifo_gettuple(ReadyList, key, rangeid);
		if (tuple)
			break;
