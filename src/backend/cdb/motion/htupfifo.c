...
htfifo_cleanup(htup_fifo htf)
{
	...
	while ((tup = htfifo_gettuple(htf, &key, &rangeid)) != NULL)
	{
		pfree(tup);
		clean_slice(key);
	}
 ...
 */
void
htfifo_addtuple(htup_fifo htf, GenericTuple tup, TupleKeySlice key, Oid rangeid)
{
	htf_entry	p_ent;
 ...
 */
GenericTuple
htfifo_gettuple(htup_fifo htf, TupleKeySlice *key, Oid *rangeid)
{
	htf_entry	p_ent;
	GenericTuple tup;
