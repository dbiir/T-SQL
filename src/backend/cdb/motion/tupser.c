 ...
 */
int
SerializeTuple(TupleTableSlot *slot, TupleKeySlice key, Oid rangeid, SerTupInfo *pSerInfo, struct directTransportBuffer *b, TupleChunkList tcList, int16 targetRoute)
{
	int			i;
	int			natts;
	...
	if (is_memtuple(gtuple)) /* memtuple case */
	{
		...
		if (CandidateForSerializeDirect(targetRoute, b))
		{
            ...
            int32		keylenByte = MEMTUP_LEAD_BIT | key.len;
            paddedSize = sizeof(keylenByte) + TYPEALIGN(TUPLE_CHUNK_ALIGN, tupleSize) + 
								TYPEALIGN(TUPLE_CHUNK_ALIGN, key.len) + TYPEALIGN(TUPLE_CHUNK_ALIGN, sizeof(Oid));
			...
			if (paddedSize + TUPLE_CHUNK_HEADER_SIZE <= b->prilen)
			{
                ...
                memcpy(pos, (char *) &keylenByte, sizeof(keylenByte));
                pos += sizeof(keylenByte);

                memcpy(pos, tuple, tupleSize);
                pos += tupleSize;
                memset(pos, 0, TYPEALIGN(TUPLE_CHUNK_ALIGN, tupleSize) - tupleSize);
                pos += TYPEALIGN(TUPLE_CHUNK_ALIGN, tupleSize) - tupleSize;

                if (key.len)
                {
                    memcpy(pos, (char *) key.data, key.len);
                    pos += key.len;
                    memset(pos, 0, TYPEALIGN(TUPLE_CHUNK_ALIGN, key.len) - key.len);
                    pos += TYPEALIGN(TUPLE_CHUNK_ALIGN, key.len) - key.len;
                }
                
                memcpy(pos, (char *) &rangeid, sizeof(rangeid));
                pos += sizeof(rangeid);
		...
		AssertState(s_tupSerMemCtxt != NULL);
        addInt32ToChunkList(tcList, MEMTUP_LEAD_BIT | key.len, &pSerInfo->chunkCache);
		...
		if (key.len > 0)
		{
			addByteStringToChunkList(tcList, (char *) key.data, key.len, &pSerInfo->chunkCache);
			addPadding(tcList, &pSerInfo->chunkCache, key.len);
		}
        addInt32ToChunkList(tcList, rangeid, &pSerInfo->chunkCache);
 ...
 */
GenericTuple
CvtChunksToTup(TupleChunkList tcList, SerTupInfo *pSerInfo, TupleRemapper *remapper, TupleKeySlice *key, Oid *rangeid)
{
	...
	/* We now have the reassembled data in 'serData'. Deserialize it back to a tuple. */
	{
		...
		if ((tshp->tuplen & MEMTUP_LEAD_BIT) != 0)
		{
			/*
			 * [williamcliu]
			 * When we send a memtuple, there may be a tuple key followed by the memtuple.
			 * Whenever a memtuple would be sent, we should move the memtuple to the
			 * second byte and set the first byte with (MEMTUP_LEAD_BIT | keylen).
			 * When we receive a memtuple, keylen equals 0 means there is not a key
			 * following the tuple, or we should read the key after tuple.
			 */
			key->len = tshp->tuplen & ~MEMTUP_LEAD_BIT;
			pos += sizeof(uint32);
			
			uint32		tuplen = memtuple_size_from_uint32(*(uint32 *) pos);
			...
			if (key->len > 0)
			{
				key->data = palloc(key->len);
				memcpy(key->data, pos, key->len);
				pos += TYPEALIGN(TUPLE_CHUNK_ALIGN, key->len);
			}
			memcpy((char*)rangeid, pos, sizeof(Oid));
			pos += TYPEALIGN(TUPLE_CHUNK_ALIGN, sizeof(Oid));
		}
		else
		{
			HeapTuple htup;
