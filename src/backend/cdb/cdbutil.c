...
getCdbComponentInfo(void)
{
	...
	component_databases->entry_db_info =
		(CdbComponentDatabaseInfo *) palloc0(sizeof(CdbComponentDatabaseInfo) * 20);
...
cdbcomponent_getComponentInfo(int contentId)
{
	...
	if (contentId < 0)
	{
		for (int i = 0; i < cdbs->total_entry_dbs; i++)
			if (contentId == cdbs->entry_db_info[i].config->segindex)
...
/*
 * Obtain the dbid of a segment at a given segment index (i.e., content id)
...
contentid_get_dbid(int16 contentid, char role, bool getPreferredRoleNotCurrentRole)
{
	...
	if (getPreferredRoleNotCurrentRole)
	{
		...
		//						  NULL, 2, scankey);
		/* we drop index GpSegmentConfigContentPreferred_roleIndexId */
		scan = systable_beginscan(rel, NULL, false, NULL, 2, scankey);
	}
	else
	{
		/*
