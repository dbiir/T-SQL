...
create_gang_retry:
	...
	PG_TRY();
	{
		for (i = 0; i < size; i++)
		{
			...
			if (cdbconn_isBadConnection(segdbDesc))
				ereport(ERROR, (errcode(ERRCODE_GP_INTERCONNECTION_ERROR),
								errmsg("failed to acquire resources on one or more segments1"),
		...
		for (;;)
		{
			...
			for (i = 0; i < size; i++)
			{
				...
				switch (pollingStatus[i])
				{
					case PGRES_POLLING_OK:
						...
						if (segdbDesc->motionListener == 0)
							ereport(ERROR, (errcode(ERRCODE_GP_INTERCONNECTION_ERROR),
											errmsg("failed to acquire resources on one or more segments2"),
					...
					case PGRES_POLLING_FAILED:
						...
						{
							ereport(ERROR, (errcode(ERRCODE_GP_INTERCONNECTION_ERROR),
											errmsg("failed to acquire resources on one or more segments3"),
											errdetail("%s (%s)", PQerrorMessage(segdbDesc->conn), segdbDesc->whoami)));
						}
						break;
					...
					default:
						ereport(ERROR, (errcode(ERRCODE_GP_INTERCONNECTION_ERROR),
										errmsg("failed to acquire resources on one or more segments4"),
				...
				if (poll_timeout == 0)
					ereport(ERROR, (errcode(ERRCODE_GP_INTERCONNECTION_ERROR),
									errmsg("failed to acquire resources on one or more segments5"),
			...
			if (nready < 0)
			{
				...
				ereport(ERROR, (errcode(ERRCODE_GP_INTERCONNECTION_ERROR),
								errmsg("failed to acquire resources on one or more segments6"),
		...
		if (successful_connections != size)
		{
			...
			if (gp_gang_creation_retry_count <= 0 ||
				...
				ereport(ERROR, (errcode(ERRCODE_GP_INTERCONNECTION_ERROR),
								errmsg("failed to acquire resources on one or more segments7"),
	...
	PG_CATCH();
	{
		...
		{
			ereport(ERROR, (errcode(ERRCODE_GP_INTERCONNECTION_ERROR),
							errmsg("failed to acquire resources on one or more segments8"),
