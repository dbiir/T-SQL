...
processResults(CdbDispatchResult *dispatchResult)
{
	...
	while (!PQisBusy(segdbDesc->conn))
	{
		...
		/*
		 * Receive the ts interval in segment.
		 */
		UpdateLowerUpperFromDtxTimeStamp(segdbDesc->conn->lts_low, segdbDesc->conn->lts_high, &global_tmp_timestamp);
        ...
        DtxTimeStamp_StartGtsUpdate(&global_tmp_timestamp, segdbDesc->conn->start_ts);
		/*
