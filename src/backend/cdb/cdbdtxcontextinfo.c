							  ...
							  int txnOptions, Snapshot snapshot)
{
    ...
    dtxContextInfo->start_ts = 0;
    dtxContextInfo->commit_ts = 0;
}
...
DtxContextInfo_SerializeSize(DtxContextInfo *dtxContextInfo)
{
    ...
    size += sizeof(uint64);     /* start time stamp */
    size += sizeof(uint64);     /* commit time stamp */
	return size;
}
...
DtxContextInfo_Serialize(char *buffer, DtxContextInfo *dtxContextInfo)
{
    ...
    memcpy(p, &dtxContextInfo->start_ts, sizeof(uint64));
	p += sizeof(uint64);
    memcpy(p, &dtxContextInfo->commit_ts, sizeof(uint64));
	p += sizeof(uint64);
}
					...
					DtxContextInfo *source)
{
    ...
    target->commit_ts = source->commit_ts;
}
						   ...
						   DtxContextInfo *dtxContextInfo)
{
	...
	if (serializedDtxContextInfolen > 0)
	{
        ...
        memcpy(&dtxContextInfo->start_ts, p, sizeof(uint64));
        p += sizeof(uint64);
        memcpy(&dtxContextInfo->commit_ts, p, sizeof(uint64));
        p += sizeof(uint64);
	}
	else
	{
