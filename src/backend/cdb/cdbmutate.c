...
static void
directDispatchCalculateRange(Plan *plan, Relation rel)
{
	int			i;
	...
	if (directDispatch)
	{
		...
		plan->directDispatch.contentIds = list_make1_int(targetID);
		plan->directDispatch.rangeid = targetRange.rangeID;
...
apply_motion_mutator(Node *node, ApplyMotionState *context)
{
	...
	switch (flow->req_move)
	{
		...
		case MOVEMENT_REPARTITION:
			...
			break;
		case MOVEMENT_RANGE:
			newnode = (Node *) make_range_motion(plan,
												  flow->hashExprs,
												  flow->hashOpfamilies,
												  true	/* useExecutorVarFormat */,
												  flow->numsegments
...
Motion *
make_range_motion(Plan *lefttree,
				   List *hashExprs,
				   List *hashOpfamilies,
				   bool useExecutorVarFormat,
				   int numsegments)
{
	...
	motion = make_motion(NULL, lefttree,
						 0, NULL, NULL, NULL, NULL, /* no ordering */
						 useExecutorVarFormat);
	add_slice_to_motion(motion, MOTIONTYPE_RANGE,
						hashExprs, hashOpfamilies, numsegments,
						false);
	return motion;
}
		...
		if (typesOK)
		{
			...
			if (root->config->gp_enable_direct_dispatch)
			{
                if (enable_range_distribution && RelationIsRocksDB(rel))
                    directDispatchCalculateRange(plan, rel);
                else
