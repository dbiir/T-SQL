...
static void XactLockTableKVWaitErrorCb(void *arg);
/*
 ...
 */
void
LockKVTuple(Relation relation, TupleKeySlice key, LOCKMODE lockmode)
{
	LOCKTAG		tag;
	...
	SET_LOCKTAG_KEY(tag,
					  relation->rd_lockInfo.lockRelId.dbId,
					  relation->rd_lockInfo.lockRelId.relId,
					  (uint32) hash_any(key.data,key.len));
...
/*
 *		ConditionalLockKVTuple
 *
 * As above, but only lock if we can get the lock without blocking.
 * Returns TRUE iff the lock was acquired.
 */
bool
ConditionalLockKVTuple(Relation relation, TupleKeySlice key, LOCKMODE lockmode)
{
	LOCKTAG		tag;
 ...
 */
void
UnlockKVTuple(Relation relation, TupleKeySlice key, LOCKMODE lockmode)
{
	LOCKTAG		tag;
 ...
 */
void
XactLockTableKVWait(TransactionId xid, Relation rel,
				  XLTW_Oper oper)
{
	LOCKTAG		tag;
	XactLockTableWaitInfo info;
	...
	if (oper != XLTW_None)
	{
		...
		callback.callback = XactLockTableKVWaitErrorCb;
		callback.arg = &info;
		callback.previous = error_context_stack;
		error_context_stack = &callback;
	}
...
void
XactLockTableStorageWait(TransactionId xid, Relation rel,
				  XLTW_Oper oper)
{
	LOCKTAG		tag;
	XactLockTableWaitInfo info;
...
*/
        ...
        bool result = TransactionIdIsInProgress(xid);
...
/*
 * XactLockTableKVWaitErrorCb
 *		Error context callback for transaction lock waits.
 */
static void
XactLockTableKVWaitErrorCb(void *arg)
{
	XactLockTableWaitInfo *info = (XactLockTableWaitInfo *) arg;
		...
		RelationIsValid(info->rel))
	{
		...
		{
			case XLTW_Update:
				cxt = gettext_noop("while updating tuple in relation \"%s\"");
				break;
			case XLTW_Delete:
				cxt = gettext_noop("while deleting tuple in relation \"%s\"");
				break;
			case XLTW_Lock:
				cxt = gettext_noop("while locking tuple in relation \"%s\"");
				break;
			case XLTW_LockUpdated:
				cxt = gettext_noop("while locking updated version of tuple in relation \"%s\"");
				break;
			case XLTW_InsertIndex:
				cxt = gettext_noop("while inserting index tuple in relation \"%s\"");
				break;
			case XLTW_InsertIndexUnique:
				cxt = gettext_noop("while checking uniqueness of tuple in relation \"%s\"");
				break;
			case XLTW_FetchUpdated:
				cxt = gettext_noop("while rechecking updated tuple in relation \"%s\"");
				break;
			case XLTW_RecheckExclusionConstr:
				cxt = gettext_noop("while checking exclusion constraint on tuple in relation \"%s\"");
				break;
