...
CreateLWLocks(void)
{
	if (!IsUnderPostmaster)
	{
		...
		for (id = 0, lock = MainLWLockArray; id < numLocks; id++, lock++)
			LWLockInitialize(&lock->lock, 0, id);
 ...
 */
void
LWLockInitialize(LWLock *lock, int tranche_id, int lock_id)
{
	SpinLockInit(&lock->mutex);
    lock->LWLock_id = lock_id;
	lock->releaseOK = true;
	lock->exclusive = 0;
	lock->shared = 0;
...
LWLockAcquireCommon(LWLock *l, LWLockMode mode, uint64 *valptr, uint64 val)
{
    ...
    if (enable_dynamic_lock)
        ADD_THREAD_LOCK_EXEC(LWinte);
	HOLD_INTERRUPTS();
    if (enable_dynamic_lock)
        REMOVE_THREAD_LOCK_EXEC(LWinte);
	/*
