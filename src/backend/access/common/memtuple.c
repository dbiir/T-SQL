...
/*
	[hzy9819]
	get the pointer to value and get the length of it
	if the value is a string, then we set len = 0
*/
char * memtuple_get_attr_value(MemTuple mtup, MemTupleBinding *pbind, int attnum, bool *isnull, Size * len) {
	bool hasnull = memtuple_get_hasnull(mtup);
	unsigned char *nullp = hasnull ? memtuple_get_nullp(mtup, pbind) : NULL; 
	char *start = (char *) mtup + (hasnull ? pbind->null_bitmap_extra_size : 0);
	...
	// ret = fetchatt(pbind->tupdesc->attrs[attnum], memtuple_get_attr_data_ptr(start, attrbind, null_saves, nullp));
	// * len = fetchattsize(pbind->tupdesc->attrs[attnum]);
	* len = pbind->tupdesc->attrs[attnum]->attlen; 
	char * value = memtuple_get_attr_data_ptr(start, attrbind, null_saves, nullp);
	if((!pbind->tupdesc->attrs[attnum]->attbyval) && ((* len) == -1))
	{
