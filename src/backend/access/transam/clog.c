...
#define CLOG_BYTES_PER_XACT 8
#define CLOG_XACTS_PER_PAGE (BLCKSZ / CLOG_BYTES_PER_XACT)

#define XactGetStatus(xact) ((* (long *) xact) >> 62)
#define XactGetEndTs(xact)	((* (long *) xact) & 0x3FFFFFFFFFFFFFFF)


#define TransactionIdToPage(xid)	((xid) / (TransactionId) CLOG_XACTS_PER_PAGE)
#define TransactionIdToPgIndex(xid) ((xid) % (TransactionId) CLOG_XACTS_PER_PAGE)
#define TransactionIdToByte(xid)	(TransactionIdToPgIndex(xid) * CLOG_BYTES_PER_XACT)
...
TransactionIdSetStatusBit(TransactionId xid, XidStatus status, XLogRecPtr lsn, int slotno)
{
	...
	/* 
	 * TODO (williamcliu): Sub transactions may have different ending timestamp  
	 * from its parent transaction.
	 */
	long record = (long) status << 62;
	if (status == TRANSACTION_STATUS_COMMITTED)
	{
		struct timeval tv;
		gettimeofday(&tv, NULL);
		// record |= tv.tv_sec;
		record |= tv.tv_sec * 1000000 + tv.tv_usec;
	}
	* (long *) byteptr = record;
...
long
TransactionIdGetEndTS(TransactionId xid)
{
	int			pageno = TransactionIdToPage(xid);
	int			byteno = TransactionIdToByte(xid);
	int			slotno;
...
TrimCLOG(void)
{
	...
	if (TransactionIdToPgIndex(xid) != 0)
	{
		...
		MemSet(byteptr, 0, BLCKSZ - byteno - 1);
