...
heapgetpage(HeapScanDesc scan, BlockNumber page)
{
	...
	kvengine_refresh_historical_kvs(false);
		 ...
		 lineoff++, lpp++)
	{
		if (ItemIdIsNormal(lpp))
		{
			...
			if (snapshot->temporal && snapshot->temporal->type == T_Temporal)
			{
				HeapTupleHeader tup = loctup.t_data;
				long xmin_ts = TransactionIdGetEndTS(HeapTupleHeaderGetXmin(tup));
				long xmax_ts = TransactionIdGetEndTS(HeapTupleHeaderGetRawXmax(tup));
				if ((!xmax_ts || xmax_ts > scan->lower) && xmin_ts < scan->upper)
					valid = true;
				else
					valid = false;
			}
		   ...
		   Relation stats_relation)
{
	 ...
	 */
	valid = HeapTupleSatisfiesVisibility(relation, tuple, snapshot, buffer, NULL);
					   ...
					   bool *all_dead, bool first_call)
{
	...
	for (;;)
	{
		...
		if (!skip)
		{
			...
			valid = HeapTupleSatisfiesVisibility(relation, heapTuple, snapshot, buffer, NULL);
					...
					ItemPointer tid)
{
	...
	for (;;)
	{
		 ...
		 * result candidate.
		 */
		valid = HeapTupleSatisfiesVisibility(relation, &tp, snapshot, buffer, NULL);
 ...
 * the original tuple.
 *
 * [williamcliu] Remove static sign.
 */
HeapTuple
heap_prepare_insert(Relation relation, HeapTuple tup, TransactionId xid,
					CommandId cid, int options, bool isFrozen)
{
	/* [williamcliu] We use haep_prepare_insert to initial rocksdb tuple. */
	Insist(RelationIsHeap(relation) || RelationIsRocksDB(relation));
...
l1:
	result = HeapTupleSatisfiesUpdate(relation, &tp, cid, buffer, NULL);
	...
	if (crosscheck != InvalidSnapshot && result == HeapTupleMayBeUpdated)
	{
		...
		if (!HeapTupleSatisfiesVisibility(relation, &tp, crosscheck, buffer, NULL))
			result = HeapTupleUpdated;
	}
	...
	if (result != HeapTupleMayBeUpdated)
	{
		...
		if (result == HeapTupleSelfUpdated)
			hufd->cmax = HeapTupleHeaderGetCmax(tp.t_data, NULL);
		else
...
l2:
	...
	result = HeapTupleSatisfiesUpdate(relation, &oldtup, cid, buffer, NULL);
	...
	{
		/* Perform additional check for transaction-snapshot mode RI updates */
		if (!HeapTupleSatisfiesVisibility(relation, &oldtup, crosscheck, buffer, NULL))
			result = HeapTupleUpdated;
	}
	...
	if (result != HeapTupleMayBeUpdated)
	{
		...
		if (result == HeapTupleSelfUpdated)
			hufd->cmax = HeapTupleHeaderGetCmax(oldtup.t_data, NULL);
		else
			hufd->cmax = 0;		/* for lack of an InvalidCommandId value */
...
l3:
	result = HeapTupleSatisfiesUpdate(relation, tuple, cid, *buffer, NULL);
...
failed:
	if (result != HeapTupleMayBeUpdated)
	{
		...
		if (result == HeapTupleSelfUpdated)
			hufd->cmax = HeapTupleHeaderGetCmax(tuple->t_data, NULL);
		else
			hufd->cmax = 0;		/* for lack of an InvalidCommandId value */
		LockBuffer(*buffer, BUFFER_LOCK_UNLOCK);
		if (have_tuple_lock)
				   ...
				   int *remaining)
{
	...
	if (nmembers >= 0)
	{
		...
		for (i = 0; i < nmembers; i++)
		{
            ...
            else if (am_kv_storage)
                XactLockTableStorageWait(memxid, rel, oper);
			else if (RelationIsRocksDB(rel))
                XactLockTableKVWait(memxid, rel, oper);
            else
...
log_heap_new_cid(Relation relation, HeapTuple tup)
{
	...
	if (hdr->t_infomask & HEAP_COMBOCID)
	{
		...
		xlrec.cmin = HeapTupleHeaderGetCmin(hdr, NULL);
		xlrec.cmax = HeapTupleHeaderGetCmax(hdr, NULL);
...
heap_xlog_clean(XLogRecPtr lsn, XLogRecord *record)
{
	...
	heap_page_prune_execute(InvalidRelation, buffer,
							redirected, nredirected,
							nowdead, ndead,
							nowunused, nunused, false);
