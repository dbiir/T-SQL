...
heap_page_prune_opt(Relation relation, Buffer buffer)
{
	...
	if (PageIsFull(page) || PageGetHeapFreeSpace(page) < minfree)
	{
		...
		if (PageIsFull(page) || PageGetHeapFreeSpace(page) < minfree)
		{
			...
			/* OK to prune */
			(void) heap_page_prune(relation, buffer, OldestXmin, true, &ignore, false);
		}
...
heap_page_prune(Relation relation, Buffer buffer, TransactionId OldestXmin,
				bool report_stats, TransactionId *latestRemovedXid, bool isvacuum)
{
	int			ndeleted = 0;
	Page		page = BufferGetPage(buffer);
	OffsetNumber offnum,
	...
	if (prstate.nredirected > 0 || prstate.ndead > 0 || prstate.nunused > 0)
	{
		...
		heap_page_prune_execute(relation, buffer,
								...
								prstate.nowunused, prstate.nunused, isvacuum);
				 ...
				 PruneState *prstate)
{
	...
	if (ItemIdIsNormal(rootlp))
	{
		...
		if (HeapTupleHeaderIsHeapOnly(htup))
		{
			 ...
			 */
			if (HeapTupleSatisfiesVacuum(relation, &tup, OldestXmin, buffer, NULL)
	...
	for (;;)
	{
		...
		switch (HeapTupleSatisfiesVacuum(relation, &tup, OldestXmin, buffer, NULL))
		{
			case HEAPTUPLE_DEAD:

				if (ItemIdHasStorage(lp) && relation->rd_pkindex != InvalidOid) {
					HeapTuple tuple = palloc0(sizeof *tuple);
					tuple->t_data = (HeapTupleHeader) PageGetItem(dp, lp);
					tuple->t_len = ItemIdGetLength(lp);
					tuple->t_iskey = true;
					TupleDesc tupDesc = CreateTupleDescCopy(RelationGetDescr(relation));
					TupleTableSlot	*slot = MakeSingleTupleTableSlot(tupDesc);
					ExecStoreHeapTuple(tuple, slot, InvalidBuffer, true);
					kvengine_his_insert(relation, slot);
 ...
 */
void
heap_page_prune_execute(Relation relation, Buffer buffer,
						OffsetNumber *redirected, int nredirected,
						OffsetNumber *nowdead, int ndead,
						OffsetNumber *nowunused, int nunused, bool isvacuum)
{
