...
ExecOpenScanRelation(EState *estate, Index scanrelid, int eflags)
{
	...
	/* append temporal info into snapshot */
	temporal = gettemporal(scanrelid, estate->es_range_table);
	estate->es_snapshot->temporal = temporal;
	// AppendTemporalInfo(temporal);
	...
	for (i = 0; i < numIndices; i++)
	{
		...
		satisfiesConstraint =
			index_insert(indexRelation, /* index relation */
						 ...
						 checkUnique,
						 slot);	/* type of uniqueness check to do */
