...
static TupleTableSlot *KVIndexNext(IndexScanState *node);
...
TupleTableSlot *
KVIndexNext(IndexScanState *node)
{
	EState	   *estate;
	ExprContext *econtext;
	ScanDirection direction;
	...
	econtext = node->ss.ps.ps_ExprContext;
	slot = node->ss.ss_ScanTupleSlot;
	KVEngineScanDesc rks_ScanDesc = node->iss_KVScanDesc;
	/*
	 * ok, now that we have what we need, fetch the next tuple.
	 */
	do
	{
		...
		kvengine_indexgetnext(rks_ScanDesc, slot);
		if (!TupIsNull(slot))
		{
			econtext->ecxt_scantuple = slot;
			ResetExprContext(econtext);
		...
		if (transam_mode == TRANSAM_MODE_NEW_OCC 
			&& 0)
		{
			AppendKeyToReadWriteSet(slot->PRIVATE_tts_key.len,
					(char *)slot->PRIVATE_tts_key.data,
					node->ss.ps.state->es_plannedstmt->commandType);

			AppendReadXidWithKey(slot->PRIVATE_tts_key.len,
					(char *)slot->PRIVATE_tts_key.data,
					node->ss.ps.state->es_plannedstmt->commandType);
		}
...
static
TupleTableSlot *
KVHisIndexNext(IndexScanState *node)
{
	EState	   *estate;
	ExprContext *econtext;
	ScanDirection direction;
...
ExecIndexScan(IndexScanState *node)
{
	...
	if (RelationIsRocksDB(node->iss_ScanDesc->heapRelation))
	{
        if (node->iss_ScanDesc->xs_snapshot == NULL || node->iss_ScanDesc->xs_snapshot->satisfies == NULL)
            ereport(LOG,
					(errmsg("INDEX SCAN: no snapshot")));
           
		if (node->iss_KVScanDesc == NULL)
			node->iss_KVScanDesc =
			kvengine_index_begin_scan(node->iss_ScanDesc);

		slot = node->ss.ss_ScanTupleSlot;

		slot = ExecScan(&node->ss,
					(ExecScanAccessMtd) KVIndexNext,
	...
	else
	{
		...
		if (TupIsNull(slot))
		{
			...
			*/
			...
			indexScanNode =  (IndexScan *)node->ss.ps.plan;
			temporal = gettemporal(indexScanNode->scan.scanrelid, node->ss.ps.state->es_range_table);

			if(temporal)
			{
				if (!node->iss_KVScanDesc)
				{
					...
					KVEngineScanDesc kvscan;
					kvscan = kvengine_history_beginscan(node->iss_ScanDesc, xmin_ts * 1000000, xmax_ts * 1000000);
					// pfree(&tmp_time);

					node->iss_KVScanDesc = kvscan;
					check_flag = true;
				}
				slot = node->ss.ss_ScanTupleSlot;

				slot = ExecScan(&node->ss,
							(ExecScanAccessMtd) KVHisIndexNext,
							(ExecScanRecheckMtd) IndexRecheck);
...
ExecEndIndexScan(IndexScanState *node)
{
	...
	kvScanDesc = node->iss_KVScanDesc;
	...
	if (kvScanDesc)
		kvengine_endscan(node->iss_KVScanDesc);
							  ...
							  Relation currentRelation, Oid indexid)
{
	...
	IndexScan *indexScanNode;
	Temporal	*temporal;
	indexScanNode =  (IndexScan *)indexstate->ss.ps.plan;
	temporal = gettemporal(indexScanNode->scan.scanrelid, indexstate->ss.ps.state->es_range_table);
	if (temporal) 
	{
		...
		pfree(lower_time);
		indexstate->iss_ScanDesc->is_temporal_query = true;
		indexstate->iss_ScanDesc->lower = lower * 1000000;
		...
		if (temporal->upper)
		{
			...
			pfree(upper_time);
			indexstate->iss_ScanDesc->upper = upper * 100000;
		}
		else
		{
			indexstate->iss_ScanDesc->upper = lower * 1000000;
		}
	}
	else
	{
		indexstate->iss_ScanDesc->is_temporal_query = false;
	}

	/*
	 * TODO: later we need init kv index scan here, also we need to init the
	 * indexscandesc->keyData
	 */
	/*if (RelationIsRocksDB(currentRelation))
	    indexstate->iss_KVScanDesc = kvengine_index_begin_scan(indexstate->iss_ScanDesc);
    else*/
        indexstate->iss_KVScanDesc = NULL;
