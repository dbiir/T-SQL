...
static TupleTableSlot *
KVIndexOnlyNext(IndexOnlyScanState *node)
{
	EState	   *estate;
	ExprContext *econtext;
	ScanDirection direction;
	...
	econtext = node->ss.ps.ps_ExprContext;
	slot = node->ss.ss_ScanTupleSlot;
	KVEngineScanDesc rks_ScanDesc = node->ioss_KVScanDesc;
	/*
	 * ok, now that we have what we need, fetch the next tuple.
	 */
	do
...
ExecIndexOnlyScan(IndexOnlyScanState *node)
{
	...
	if (RelationIsRocksDB(node->ioss_ScanDesc->heapRelation))
	{
		if (node->ioss_KVScanDesc == NULL)
			node->ioss_KVScanDesc =
			kvengine_index_begin_scan(node->ioss_ScanDesc);
		TupleTableSlot *slot;
		slot = node->ss.ss_ScanTupleSlot;

		slot = ExecScan(&node->ss,
					(ExecScanAccessMtd) KVIndexOnlyNext,
...
ExecEndIndexOnlyScan(IndexOnlyScanState *node)
{
	...
	kvScanDesc = node->ioss_KVScanDesc;
	...
	if (kvScanDesc)
		kvengine_endscan(node->ioss_KVScanDesc);
...
ExecInitIndexOnlyScan(IndexOnlyScan *node, EState *estate, int eflags)
{
	...
	/*if (RelationIsRocksDB(currentRelation))
	    indexstate->ioss_KVScanDesc = kvengine_index_begin_scan(indexstate->ioss_ScanDesc);
    else*/
        indexstate->ioss_KVScanDesc = NULL;
