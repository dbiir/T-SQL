...
ExecProject(ProjectionInfo *projInfo, ExprDoneCond *isDone)
{
	...
	if (numSimpleVars > 0)
	{
		...
		if (projInfo->pi_directMap)
		{
			...
			for (i = 0; i < numSimpleVars; i++)
			{
				...
				/* [williamcliu] copy rocks key if has */
				ExecCopyTupleKeyIfHas(slot, varSlot);
			}
		}
		else
		{
			...
			for (i = 0; i < numSimpleVars; i++)
			{
				...
				/* [williamcliu] copy rocks key if has (Maybe in the wrong place) */
				ExecCopyTupleKeyIfHas(slot, varSlot);
			}
		}
	}
	...
	{
		if (slot && econtext->ecxt_scantuple)
			ExecCopyTupleKeyIfHas(slot, econtext->ecxt_scantuple);
	}
