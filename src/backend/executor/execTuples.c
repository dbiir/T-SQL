...
/* --------------------------------
*	[williamcliu]
*		ExecCopyTupleKeyIfHas
*		 	Copy RocksDB Key from src to dest.
* --------------------------------
*/
bool
ExecCopyTupleKeyIfHas(TupleTableSlot* dest, TupleTableSlot* src)
{
	Assert(dest && src);
	if (TupHasRocksTuple(dest) && TupHasRocksTuple(src) && dest->PRIVATE_tts_key.data != src->PRIVATE_tts_key.data)
	{
		free_key(dest);
		free_key(src);
		return false;
	}
	if (!TupHasRocksTuple(dest) && TupHasRocksTuple(src))
	{
		if (TupShouldFreeKey(src))
			TupSetShouldFreeKey(dest);
		else
			TupClearShouldFreeKey(dest);
		TupClearShouldFreeKey(src);
		dest->PRIVATE_tts_key = src->PRIVATE_tts_key;
	}
	return true;
}
		...
		*result = 0;
        ...
        else
        {
			Assert(TupHasMemTuple(slot) || TupHasVirtualTuple(slot));
                ...
                {
                        case SelfItemPointerAttributeNumber:
							Assert(TupHasRocksTuple(slot) || ItemPointerIsValid(&(slot->PRIVATE_tts_synthetic_ctid)));
