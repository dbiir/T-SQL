		   ...
		   Oid	tupleOid)
{
	...
	rel_is_rocksdb = RelationIsRocksDB(resultRelationDesc);
	...
	else if (rel_is_rocksdb)
	{
		if (resultRelInfo->ri_kvInsertDesc == NULL)
			resultRelInfo->ri_kvInsertDesc = kvengine_insert_init(resultRelationDesc);
	}
	...
	else
	{
		...
		{
			//kvengine_check_unique(resultRelationDesc, 0, slot, NULL);
			if (resultRelInfo->ri_kvInsertDesc == NULL)
				resultRelInfo->ri_kvInsertDesc = kvengine_insert_init(resultRelationDesc);

 			newId = kvengine_insert(resultRelInfo->ri_kvInsertDesc, slot, estate->es_output_cid);
			//newId = InvalidOid;
			ItemPointerSetInvalid(&lastTid);
		}
		else
		   ...
		   TupleKeySlice rocks_key)
{
	...
	bool isRocksDBTable = RelationIsRocksDB(resultRelationDesc);
...
ldelete:;
		...
		else if (isRocksDBTable)
		{
			Assert(rocks_key.data && rocks_key.len);
			
			if (resultRelInfo->ri_deleteDesc == NULL)
				resultRelInfo->ri_deleteDesc = kvengine_delete_init(resultRelationDesc);
			
			result = kvengine_delete(resultRelInfo->ri_deleteDesc,
						rocks_key,
						estate->es_output_cid,
						estate->es_crosscheck_snapshot,
                        &rufd);
		...
		switch (result)
		{
			case HeapTupleSelfUpdated:
                ...
                /* 
                 * TODO: [HONGYAOZHAO CHARLYZHAO]
                 * In order to prevent reporting errors, this error reporting has 
                 * been suspended for the time being.
                 */

				if (!isAORowsTable && !isAOColsTable && 
                    !isRocksDBTable && hufd.cmax != estate->es_output_cid)
					ereport(ERROR,
							(errcode(ERRCODE_TRIGGERED_DATA_CHANGE_VIOLATION),
							 errmsg("tuple to be updated was already modified by an operation triggered by the current command 841"),
			...
			case HeapTupleUpdated:
				...
				if (isRocksDBTable &&
                    memcmp(rufd.key.data, rocks_key.data, get_min(rufd.key.len, rocks_key.len)) != 0)
				{
                    /*ereport(LOG,
					(errmsg("DELETE: EvalPlanQual rufd is not equal to the old key, retry_time %d", retry_time)));*/
                    TupleTableSlot *epqslot;

					epqslot = EvalRocksPlanQual(estate,
										   epqstate,
										   &rufd.key);
                    retry_time++;
					if (!TupIsNull(epqslot))
					{
						rocks_key.data = rufd.key.data;
                        rocks_key.len = rufd.key.len;
						slot = ExecFilterJunk(resultRelInfo->ri_junkFilter, epqslot);
						// slot = epqslot;
                        goto ldelete;
					}
                    /*ereport(LOG,
					(errmsg("DELETE: no found new key value")));*/
                    return NULL;
				}
				else if (!isRocksDBTable && !ItemPointerEquals(tupleid, &hufd.ctid))
				{
					TupleTableSlot *epqslot;
                ...
                /*ereport(LOG,
					(errmsg("DELETE: EvalPlanQual rufd is equal to the old key!")));*/
				return NULL;
		   ...
		   TupleKeySlice rocks_key)
{
	...
	bool		rel_is_rocksdb = RelationIsRocksDB(resultRelationDesc);
	...
	if (rel_is_aorows || rel_is_aocols || rel_is_rocksdb)
	{
		/*
...
lreplace:;
		...
		{		
			if (resultRelInfo->ri_updateDesc == NULL)
				resultRelInfo->ri_updateDesc = kvengine_update_init(resultRelationDesc);
			
			result = kvengine_update(resultRelInfo->ri_updateDesc,
						slot,
						rocks_key,
						estate->es_output_cid,
						estate->es_crosscheck_snapshot,
		...
		switch (result)
		{
			case HeapTupleSelfUpdated:
                 ...
                 * been suspended for the time being.
                 */
				if (!rel_is_aorows && !rel_is_aocols && !rel_is_rocksdb && hufd.cmax != estate->es_output_cid)
					ereport(ERROR,
							(errcode(ERRCODE_TRIGGERED_DATA_CHANGE_VIOLATION),
							 errmsg("tuple to be updated was already modified by an operation triggered by the current command 1527"),
							 errhint("Consider using an AFTER trigger instead of a BEFORE trigger to propagate changes to other rows.")));
                else if (rel_is_rocksdb && rufd.cmax != estate->es_output_cid)
                {
                    ereport(LOG,
							(errcode(ERRCODE_TRIGGERED_DATA_CHANGE_VIOLATION),
							 errmsg("tuple to be updated was already modified by an operation triggered by the current command 1546"),
							 errhint("Consider using an AFTER trigger instead of a BEFORE trigger to propagate changes to other rows.")));
                    break;
                }
                else
                    ereport(LOG,
					    (errmsg("UPDATE: already updated by self; nothing to do")));               
			...
			case HeapTupleUpdated:
				...
				if (rel_is_rocksdb && rufd.key.len == rocks_key.len) 
                    if (memcmp(rufd.key.data, rocks_key.data, get_min(rufd.key.len, rocks_key.len)) != 0)
                    {                        
                        TupleTableSlot *epqslot;
                        ...
                        ereport(LOG,
                        (errmsg("UPDATE: no found new key value, retry time %d", retry_time)));
                        return NULL;
				    }
                    else
                    {
                        ereport(LOG,
                        (errmsg("UPDATE: return key len not equal")));
                        return NULL;
                    }
				else if (!rel_is_rocksdb && !ItemPointerEquals(tupleid, &hufd.ctid))
				{
                    /*ereport(LOG,
					(errmsg("UPDATE: EvalPlanQual hufd ctid block hi:%d, block lo:%d, posid %d; old tupleid block hi:%d, block lo:%d, posid %d", 
                    hufd.ctid.ip_blkid.bi_hi, hufd.ctid.ip_blkid.bi_lo, hufd.ctid.ip_posid, 
                    tupleid->ip_blkid.bi_hi, tupleid->ip_blkid.bi_lo, tupleid->ip_posid)));*/
				...
				/* tuple already deleted; nothing to do */
                ereport(LOG,
					(errmsg("UPDATE: EvalPlanQual rufd key is NULL!")));
				return NULL;
	...
	if (resultRelInfo->ri_projectReturning)
	{
		...
		tmp_slot = ExecProcessReturning(resultRelInfo->ri_projectReturning,
									slot, planSlot);
		if (transam_mode != TRANSAM_MODE_DEFAULT && rocks_key.data)
			tmp_slot->PRIVATE_tts_key = rocks_key;
		return tmp_slot;
	}
			...
			case CMD_UPDATE:
				{
					...
					if (actionColIdx <= 0)
					{
						TupleKeySlice key = planSlot->PRIVATE_tts_key;
