...
set_plain_rel_pathlist(PlannerInfo *root, RelOptInfo *rel, RangeTblEntry *rte)
{
	...
	/*
	 * TODO: I forced rocksdb's query to be changed to an index query,
	 * which may need to be modified later.
	 */
	if (rel->relstorage == RELSTORAGE_ROCKSDB)
	{
		int path_length = list_length(rel->pathlist);
		if (path_length == 0)
			add_path(rel, seqpath);
	}
	else if (rel->relstorage != RELSTORAGE_ROCKSDB)
