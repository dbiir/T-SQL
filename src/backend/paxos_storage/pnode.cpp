...
int PNode :: InitLogStorage(const Options & oOptions, LogStorage *& poLogStorage, int iGroupIndex)
{
    ...
    int ret = m_oDefaultLogStorage.Init(oOptions.sLogStoragePath, iGroupIndex);
    if (ret != 0)
    {
...
void PNode :: RunMaster(const Options & oOptions)
{
    for (auto & oGroupSMInfo : oOptions.vecGroupSMInfoList)
    {
        ...
        {
            if (!m_vecGroupList[0]->GetConfig()->IsIMFollower())
            {
                m_vecMasterList[0]->RunMaster();
            }
            else
            {
...
int PNode :: Init(int iGroupIndex,const Options & oOptions, NetWork *& poNetWork)
{
...
//    }
    ...
    int ret = InitLogStorage(oOptions, poLogStorage,iGroupIndex);
    if (ret != 0)
    {
        return ret;
    }
    ...
    for (int iGroupIdx = iGroupIndex; iGroupIdx < iGroupIndex+1; iGroupIdx++)
    {
        Group * poGroup = new Group(poLogStorage, poNetWork, m_vecMasterList[0]->GetMasterSM(), iGroupIdx, oOptions);
...
int PNode :: Propose(const int iGroupIdx, const std::string & sValue, uint64_t & llInstanceID)
{
    ...
    return m_vecGroupList[0]->GetCommitter()->NewValueGetID(sValue, llInstanceID);
}
...
int PNode :: Propose(const int iGroupIdx, const std::string & sValue, uint64_t & llInstanceID, SMCtx * poSMCtx)
{
    ...
    return m_vecGroupList[0]->GetCommitter()->NewValueGetID(sValue, llInstanceID, poSMCtx);
}
...
const uint64_t PNode :: GetNowInstanceID(const int iGroupIdx)
{
    ...
    return m_vecGroupList[0]->GetInstance()->GetNowInstanceID();
}
...
const uint64_t PNode :: GetMinChosenInstanceID(const int iGroupIdx)
{
    ...
    return m_vecGroupList[0]->GetInstance()->GetMinChosenInstanceID();
}
...
int PNode :: OnReceiveMessage(const char * pcMessage, const int iMessageLen)
{
    ...
    return m_vecGroupList[0]->GetInstance()->OnReceiveMessage(pcMessage, iMessageLen);
}
...
void PNode :: AddStateMachine(const int iGroupIdx, StateMachine * poSM)
{
    ...
    m_vecGroupList[0]->AddStateMachine(poSM);
}
...
int PNode :: AddMember(const int iGroupIdx, const NodeInfo & oNode)
{
    ...
    SystemVSM * poSystemVSM = m_vecGroupList[0]->GetConfig()->GetSystemVSM();
...
const NodeInfo PNode :: GetMaster(const int iGroupIdx)
{
    ...
    return NodeInfo(m_vecMasterList[0]->GetMasterSM()->GetMaster());
}
...
const NodeInfo PNode :: GetMasterWithVersion(const int iGroupIdx, uint64_t & llVersion) 
{
    ...
    return NodeInfo(m_vecMasterList[0]->GetMasterSM()->GetMasterWithVersion(llVersion));
}
...
const bool PNode :: IsIMMaster(const int iGroupIdx)
{
    ...
    return m_vecMasterList[0]->GetMasterSM()->IsIMMaster();
}
...
int PNode :: SetMasterLease(const int iGroupIdx, const int iLeaseTimeMs)
{
    ...
    m_vecMasterList[0]->SetLeaseTime(iLeaseTimeMs);
    return 0;
}
...
void PNode :: SetMaxHoldThreads(const int iGroupIdx, const int iMaxHoldThreads)
{
    ...
    m_vecGroupList[0]->GetCommitter()->SetMaxHoldThreads(iMaxHoldThreads);
}
...
void PNode :: SetProposeWaitTimeThresholdMS(const int iGroupIdx, const int iWaitTimeThresholdMS)
{
    ...
    m_vecGroupList[0]->GetCommitter()->SetProposeWaitTimeThresholdMS(iWaitTimeThresholdMS);
}
...
void PNode :: SetLogSync(const int iGroupIdx, const bool bLogSync)
{
    ...
    m_vecGroupList[0]->GetConfig()->SetLogSync(bLogSync);
}
        ...
        std::vector<std::pair<std::string, int> > & vecValues)
{
    ...
    string sValue;
    int iSMID = 0;
    int ret = m_vecGroupList[0]->GetInstance()->GetInstanceValue(llInstanceID, sValue, iSMID);
    if (ret != 0)
    {
        return ret;
    }
        ...
        uint64_t & llInstanceID, uint32_t & iBatchIndex, SMCtx * poSMCtx)
{
    ...
    return m_vecProposeBatch[0]->Propose(sValue, llInstanceID, iBatchIndex, poSMCtx);
}
...
void PNode :: SetBatchCount(const int iGroupIdx, const int iBatchCount)
{
    ...
    m_vecProposeBatch[0]->SetBatchCount(iBatchCount);
}
...
void PNode :: SetBatchDelayTimeMs(const int iGroupIdx, const int iBatchDelayTimeMs)
{
    ...
    m_vecProposeBatch[0]->SetBatchDelayTimeMs(iBatchDelayTimeMs);
}
