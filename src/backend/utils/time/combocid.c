...
static CommandId GetRealCmin(TransactionId xmin, CommandId combocid, SessionMessage sm);
static CommandId GetRealCmax(TransactionId xmin, CommandId combocid, SessionMessage sm);
...
static void loadSharedComboCommandId(TransactionId xmin, CommandId combocid, CommandId *cmin, CommandId *cmax, SessionMessage sm);
...
CommandId
HeapTupleHeaderGetCmin(HeapTupleHeader tup, SessionMessage sm)
{
	...
	if (tup->t_infomask & HEAP_COMBOCID)
		return GetRealCmin(HeapTupleHeaderGetXmin(tup), cid, sm);
	else
		return cid;
}

CommandId
HeapTupleHeaderGetCmax(HeapTupleHeader tup, SessionMessage sm)
{
	CommandId	cid = HeapTupleHeaderGetRawCommandId(tup);
	...
	if (tup->t_infomask & HEAP_COMBOCID)
		return GetRealCmax(HeapTupleHeaderGetXmin(tup), cid, sm);
	else
		return cid;
}
						  ...
						  bool *iscombo)
{
	...
	{
		CommandId	cmin = HeapTupleHeaderGetCmin(tup, NULL);
...
static CommandId
getSharedComboCidEntry(TransactionId xmin, CommandId combocid, enum minmax min_or_max, SessionMessage sm)
{
	bool		found;
	...
	{
		loadSharedComboCommandId(xmin, combocid, &cmin, &cmax, sm);
	}
...
static CommandId
GetRealCmin(TransactionId xmin, CommandId combocid, SessionMessage sm)
{
	if (combocid >= usedComboCids)
	{
		...
		/* We're a reader */
		return getSharedComboCidEntry(xmin, combocid, CMIN, sm);
	}
...
static CommandId
GetRealCmax(TransactionId xmin, CommandId combocid, SessionMessage sm)
{
	if (combocid >= usedComboCids)
	{
		if (Gp_is_writer)
		...
		/* We're a reader */
		return getSharedComboCidEntry(xmin, combocid, CMAX, sm);
	}
...
void
loadSharedComboCommandId(TransactionId xmin, CommandId combocid, CommandId *cmin, CommandId *cmax, SessionMessage sm)
{
	bool		found = false;
	ComboCidEntryData entry;
	int			i;
	int session_id = gp_session_id;
	int pid = lockHolderProcPtr->pid;
	int combocid_map_count = lockHolderProcPtr->combocid_map_count;
	if (sm != NULL)
	{
		session_id = sm->gp_session_id;
		pid = sm->pid;
		combocid_map_count = sm->combocid_map_count;
	}
	...
	if (combocid_map == NULL || am_kv_storage)
	{
		MemoryContext oldCtx;
		char			path[MAXPGPATH];

		ComboCidMapName(path, session_id, pid);
		/* open our file, as appropriate: this will throw an error if the create-fails. */
		oldCtx = MemoryContextSwitchTo(TopMemoryContext);
	...
	for (i = 0; i < combocid_map_count; i++)
	{
		...
		if (entry.key.xmin == xmin)
		{
			...
			memset(&reader_key, 0, sizeof(reader_key));
			reader_key.writer_pid = pid;
			reader_key.xmin = entry.key.xmin;
			reader_key.session = session_id;
