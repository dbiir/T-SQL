			...
			uint16 infomask, TransactionId xid)
{
	...
	if (TransactionIdIsValid(xid))
	{
		...
		/* [williamcliu] Only if relation storage is not RocksDB, do we need flush to disk. */
		if (XLogNeedsFlush(commitLSN) && !RelationIsRocksDB(rel) && BufferIsPermanent(buffer))
 ...
 */
bool
HeapTupleSatisfiesSelf(Relation relation, HeapTuple htup, Snapshot snapshot, Buffer buffer, SessionMessage sm)
{
 ...
 */
bool
HeapTupleSatisfiesAny(Relation relation, HeapTuple htup, Snapshot snapshot, Buffer buffer, SessionMessage sm)
{
	return true;
}
...
HeapTupleSatisfiesToast(Relation relation, HeapTuple htup, Snapshot snapshot,
						Buffer buffer, SessionMessage sm)
{
	HeapTupleHeader tuple = htup->t_data;
						 ...
						 Buffer buffer, SessionMessage sm)
{
	...
	Assert(ItemPointerIsValid(&htup->t_self) || RelationIsRocksDB(relation));
#if 0
	Assert(htup->t_tableOid != InvalidOid);
#endif
	...
	if (!HeapTupleHeaderXminCommitted(tuple))
	{
		...
		{
			if (HeapTupleHeaderGetCmin(tuple, sm) >= curcid)
            {
                core_dump();
			...
			if (tuple->t_infomask & HEAP_XMAX_IS_MULTI)
			{
				...
				{
					if (HeapTupleHeaderGetCmax(tuple, sm) >= curcid)
					   ...
					   Buffer buffer, SessionMessage sm)
{
	...
	if (!HeapTupleHeaderXminCommitted(tuple))
	{
		...
		{
			if (HeapTupleHeaderGetCmin(tuple, sm) >= snapshot->curcid)
			...
			if (tuple->t_infomask & HEAP_XMAX_IS_MULTI)
			{
				...
				if (!TransactionIdIsCurrentTransactionId(xmax))
					return true;
				else if (HeapTupleHeaderGetCmax(tuple, sm) >= snapshot->curcid)
			...
			if (HeapTupleHeaderGetCmax(tuple, sm) >= snapshot->curcid)
