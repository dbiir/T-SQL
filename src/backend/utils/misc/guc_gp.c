...
/* Executor */
bool gp_enable_mk_sort = false;
bool gp_enable_motion_mk_sort = false;
...
static const struct config_enum_entry transam_modes[] = {
	{"default", TRANSAM_MODE_DEFAULT},
	{"occ", TRANSAM_MODE_OCC},
	{"docc", TRANSAM_MODE_BOCC},
	{"rucc", TRANSAM_MODE_NEW_OCC},
	{NULL, TRANSAM_MODE_DEFAULT}};

int consistency_mode;
static const struct config_enum_entry consistency_modes[] = {
	{"default", CONSISTENCY_MODE_DEFAULT},
	{"causal", CONSISTENCY_MODE_CAUSAL},
	{"sequence", CONSISTENCY_MODE_SEQUENCE},
	{"rucc", CONSISTENCY_MODE_RUCC},
	{NULL, CONSISTENCY_MODE_DEFAULT}};

int transaction_op_type;
static const struct config_enum_entry transaction_types[] = {
	{"opt", TRANSACTION_TYPE_O},
	{"pre", TRANSACTION_TYPE_P},
	{"default", TRANSACTION_TYPE_O},
	{NULL, TRANSACTION_TYPE_O}};

int rocksdb_scan_foward;
static const struct config_enum_entry rocksdb_scan_fowards[] = {
	{"forward", ROCKSDBSCAN_FORWARD},
	{"backward", ROCKSDBSCAN_BACKWARD},
	{"default", ROCKSDBSCAN_FORWARD},
	{NULL, ROCKSDBSCAN_FORWARD}};

int dta_rollback_count;
static const struct config_enum_entry dta_rollback_counts[] = {
	{"10000", ROLLBACKCOUNT_10000},
	{"100000", ROLLBACKCOUNT_100000},
	{"1000000", ROLLBACKCOUNT_1000000},
	{"10000000", ROLLBACKCOUNT_10000000},
	{NULL, ROLLBACKCOUNT_100000}};

int http_delay;
static const struct config_enum_entry http_delays[] = {
	{"0", HTTP_DELAY_0MS},
	{"0.3", HTTP_DELAY_03MS},
	{"1", HTTP_DELAY_1MS},
	{"3", HTTP_DELAY_3MS},
...
/* raffertyyu: enable use rocksdb global */
int store_mode;
static const struct config_enum_entry store_modes[] = {
	{"heap", STORE_MODE_HEAP},
	{"rocksdb", STORE_MODE_ROCKSDB},
	{NULL, 0}};
...
struct config_bool ConfigureNamesBool_gp[] =
	{
		...
		{{"gp_write_shared_snapshot", PGC_USERSET, UNGROUPED,
		 ...
		 false,
		 NULL,
		 assign_gp_write_shared_snapshot,
		 NULL},
		...
		{{"debug_dtm_action_primary", PGC_SUSET, DEVELOPER_OPTIONS,
		 ...
		 DEBUG_DTM_ACTION_PRIMARY_DEFAULT,
		 NULL,
		 NULL,
		 NULL},
		...
		{{"pljava_classpath_insecure", PGC_POSTMASTER, CUSTOM_OPTIONS,
		 ...
		 false,
		 check_pljava_classpath_insecure,
		 assign_pljava_classpath_insecure,
		 NULL},
		...
		{{"gp_resource_group_bypass", PGC_USERSET, RESOURCES,
		 ...
		 false,
		 check_gp_resource_group_bypass,
		 NULL,
		 NULL},
...
struct config_int ConfigureNamesInt_gp[] =
	{
		...
		{{"debug_dtm_action_segment", PGC_SUSET, DEVELOPER_OPTIONS,
		 ...
		 DEBUG_DTM_ACTION_SEGMENT_DEFAULT,
		 -2,
		 1000,
		 NULL,
		 NULL,
		...
		{{"debug_dtm_action_nestinglevel", PGC_SUSET, DEVELOPER_OPTIONS,
		 ...
		 DEBUG_DTM_ACTION_NESTINGLEVEL_DEFAULT,
		 0,
		 1000,
		 NULL,
		 NULL,
		...
		{{"gp_vmem_idle_resource_timeout", PGC_USERSET, CLIENT_CONN_OTHER,
		 ...
		 600000,
		 0,
		 INT_MAX, /* 10 minutes by default on debug
										 * builds. */
#else
		 18000,
		 0,
	 ...
	 */
		...
		{{"gp_hashagg_default_nbatches", PGC_USERSET, QUERY_TUNING_METHOD,
		 ...
		 4,
		 1048576,
		 check_gp_hashagg_default_nbatches,
		 NULL,
		 NULL},
		...
		{{"gp_max_slices", PGC_USERSET, PRESET_OPTIONS,
		 ...
		 NULL,
		 NULL},
		/* [williamcliu] historical transfer */
		{
			{"completed_historical_kv_interval", PGC_USERSET, DEVELOPER_OPTIONS,
				gettext_noop("Put N different historical kvs between completed historical kvs."),
				NULL,
				GUC_NOT_IN_SAMPLE
			},
			&CompletedHistoricalKVInterval,
			100, 
			0, 
			INT_MAX / 2,
			NULL, 
...
struct config_string ConfigureNamesString_gp[] =
	{
		...
		{{"gp_session_role", PGC_BACKEND, GP_WORKER_IDENTITY,
		 ...
		 "dispatch",
		 check_gp_session_role,
		 assign_gp_session_role,
		 show_gp_session_role},
		...
		{{"gp_resqueue_priority_default_value", PGC_POSTMASTER, RESOURCES_MGM,
		 ...
		 "MEDIUM",
		 gpvars_check_gp_resqueue_priority_default_value,
		 NULL,
		 NULL},
		...
		{{"gp_default_storage_options", PGC_USERSET, APPENDONLY_TABLES,
		 ...
		 assign_gp_default_storage_options,
		 NULL},
...
struct config_enum ConfigureNamesEnum_gp[] =
	{
		...
		{{"debug_dtm_action_protocol", PGC_SUSET, DEVELOPER_OPTIONS,
		 ...
		 debug_dtm_action_protocol_options,
		 NULL,
		 NULL,
		 NULL},
		...
		{{"password_hash_algorithm", PGC_SUSET, CONN_AUTH_SECURITY,
		 ...
		 PASSWORD_HASH_MD5,
		 password_hash_algorithm_options,
		 NULL,
		 NULL,
		 NULL},
		...
		{{"explain_memory_verbosity", PGC_USERSET, RESOURCES_MEM,
		 ...
		 EXPLAIN_MEMORY_VERBOSITY_SUPPRESS,
		 explain_memory_verbosity_options,
		 NULL,
		 NULL,
		 NULL},
		...
		{{"debug_dtm_action", PGC_SUSET, DEVELOPER_OPTIONS,
		  ...
		  GUC_SUPERUSER_ONLY | GUC_NO_SHOW_ALL | GUC_NOT_IN_SAMPLE},
		 &Debug_dtm_action,
		 DEBUG_DTM_ACTION_NONE,
		 debug_dtm_action_options,
		 NULL,
		 NULL,
		 NULL},
		...
		{{"debug_dtm_action_target", PGC_SUSET, DEVELOPER_OPTIONS,
		 ...
		 DEBUG_DTM_ACTION_TARGET_NONE,
		 debug_dtm_action_target_options,
		 NULL,
		 NULL,
		 NULL},
		...
		{{"optimizer_join_order", PGC_USERSET, QUERY_TUNING_OTHER,
		 ...
		 JOIN_ORDER_EXHAUSTIVE_SEARCH,
		 optimizer_join_order_options,
		 NULL,
		 NULL,
		 NULL},

		/* charleyzhao: transam mode GNCs */
		{
			{"transam_mode", PGC_USERSET, DEVELOPER_OPTIONS,
			 gettext_noop("Set transaction manager's mode"),
			 gettext_noop("current support 'default' and 'occ'."),
			 GUC_NOT_IN_SAMPLE},
			&transam_mode,
			...
			NULL,
			NULL},
		/* hongyaozhao: consistency mode GNCs */
		{
			{"consistency_mode", PGC_USERSET, DEVELOPER_OPTIONS,
			 gettext_noop("Set consistency manager's mode"),
			 gettext_noop("current support 'default' and 'casual'."),
			 GUC_NOT_IN_SAMPLE},
			&consistency_mode,
			...
			NULL,
			NULL},
		{{"transaction_type", PGC_USERSET, DEVELOPER_OPTIONS,
		  gettext_noop("Set consistency manager's mode"),
		  gettext_noop("current support 'Optimistic' and 'Pessimistic'."),
		  GUC_NOT_IN_SAMPLE},
		 &transaction_op_type,
		 TRANSACTION_TYPE_O,
		 transaction_types,
		 ...
		 NULL,
		 NULL},
		{{"rocksdb_scan_forward", PGC_USERSET, DEVELOPER_OPTIONS,
		  gettext_noop("Set consistency manager's mode"),
		  gettext_noop("current support 'Optimistic' and 'Pessimistic'."),
		  GUC_NOT_IN_SAMPLE},
		 &rocksdb_scan_foward,
		 ROCKSDBSCAN_FORWARD,
		 rocksdb_scan_fowards,
		 NULL,
		 NULL,
		 NULL},
		{{"dta_rollback_count", PGC_USERSET, DEVELOPER_OPTIONS,
		  gettext_noop("Set consistency manager's mode"),
		  gettext_noop("current support '1w' and '10w' '100w' '1000w'."),
		  GUC_NOT_IN_SAMPLE},
		 &dta_rollback_count,
		 ...
		 NULL,
		 NULL},
		{{"http_delay", PGC_USERSET, DEVELOPER_OPTIONS,
		  gettext_noop("Set http delay"),
		  gettext_noop("current support '0' '1' '3' and '5'."),
		  GUC_NOT_IN_SAMPLE},
		 &http_delay,
		 HTTP_DELAY_0MS,
		 http_delays,
		 ...
		 NULL,
		 NULL},
		/* raffertyyu: use rocksdb global */
		{
			{"store_mode", PGC_USERSET, DEVELOPER_OPTIONS,
			 gettext_noop("Set if always use rocksdb table"),
			 gettext_noop("current support 'rocksdb'."),
			 GUC_NOT_IN_SAMPLE},
			&store_mode,
			STORE_MODE_HEAP,
			store_modes,
...
bool check_gp_hashagg_default_nbatches(int *newval, void **extra, GucSource source)
{
 ...
 */
void set_gp_replication_config(const char *name, const char *value)
{
