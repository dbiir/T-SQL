 ...
 */
uint32
gxid_hash(const void *key, Size keysize)
{
	Assert(keysize == sizeof(Oid));
	uint32 orig_key = (uint32) *((const Oid *) key);
	uint32 hash_key = orig_key / PENGUINDB_GLOBAL_MASTER_NUMBER / TXN_BUCKET;
	return DatumGetUInt32(hash_uint32(hash_key));
}
