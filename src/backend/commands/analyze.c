 ...
 */
static int
acquire_sample_rows_rocks(Relation onerel, int elevel,
						 HeapTuple *rows, int targrows,
						 double *totalrows, double *totaldeadrows)
{
	/* TODO: write a function aboout rocksdb table */
	...
	KVEngineScanDesc scan = kvengine_beginscan(onerel, NULL);
	...
	while (false)
	{
		...
		HeapTuple targtuple = kvengine_make_fake_heap(key, value);
		...
		kvengine_free_fake_heap(targtuple);
		++(scan->scan_index);
	}
					...
					double *totalrows, double *totaldeadrows)
{
	...
	/* Gather sample on this server. */
		...
		return acquire_sample_rows_ao(onerel, elevel, rows, targrows,
									  totalrows, totaldeadrows);
	else if (RelationIsRocksDB(onerel))
	{
		return acquire_sample_rows_rocks(onerel, elevel, rows, targrows,
										totalrows, totaldeadrows);
	}
	else
