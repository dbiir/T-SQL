...
DefineRelation(CreateStmt *stmt, char relkind, Oid ownerId, char relstorage, bool dispatch, bool useChangedOpts, GpPolicy *intoPolicy)
{
	...
	*/
	...
	{
		RangeDesc rangedesc = initNewTableRangeDesc(rel, stmt->relRangeID, stmt->replica_segid, stmt->replicanum);
		// store the route kv(only one level)
		storeNewRangeRoute(rangedesc);

		// if the user's data of the range will store at current seg, store the range meta data here
		if (Gp_role == GP_ROLE_EXECUTE)
		{
			SegmentID currentSeg = GpIdentity.segindex;
			for (int i = 0; i < rangedesc.replica_num; i++)
			{
			...
			//RangeDesc testRangeDesc = findUpRangeDescByID(rangedesc.rangeID);
		}
		stmt->relRangeID = rangedesc.rangeID;
		stmt->replicanum = rangedesc.replica_num;
		stmt->replica_segid = palloc0(sizeof(SegmentID) * rangedesc.replica_num);
		for (int i = 0; i < rangedesc.replica_num; i++)
		{
	...
	{
		stmt->relRangeID = UNVALID_RANGE_ID;
	}
	/*
	...
	if (inhSchema != NIL)
	{
		foreach(entry, schema)
		{
			...
			if (exist_attno > 0)
			{
				 ...
				 * have the same type, typmod, and collation.
				 */
				if (strcmp(attributeName, "inner_primary_key"))
...
ATRewriteTables(List **wqueue, LOCKMODE lockmode)
{
	...
	foreach(ltab, *wqueue)
	{
		...
		if (tab->rewrite)
		{
			/* [williamcliu] if relation is RocksDB, we need not make a new heap. */
			if (relstorage == RELSTORAGE_ROCKSDB)
			{
...
ATRewriteTable(AlteredTableInfo *tab, Oid OIDNewHeap, LOCKMODE lockmode)
{
	...
	notnull_attrs = NIL;
	/* [williamcliu] replace newrel to rewrite */
	...
	if (tab->rewrite || needscan)
	{
		...
		else if (relstorage_is_rocksdb(relstorage))
		{
			Assert(!newrel);
			KVEngineInsertDesc	kvInsertDesc = NULL;

			if (tab->rewrite)
			{
				kvInsertDesc = kvengine_insert_init(oldrel);
			}
		}
		else
		{
