...
static int  getMSHeartBeat(PGconn *conn, int msgLength);
static int  getSplitPreparePlan(PGconn *conn, int msgLength);
...
pqParseInput3(PGconn *conn)
{
	...
	for (;;)
	{
		...
		else
		{
			...
			switch (id)
			{
				...
				case 'M':
					{
						int type = 0;
						pqGetInt(&type, 4, conn);
						switch (type)
						{
							case MS_HEART_BEAT:
								getMSHeartBeat(conn, msgLength);
								break;
							case MS_SPLIT_PREPARE:
								getSplitPreparePlan(conn, msgLength);
								break;
							case MS_SPLIT_SUCCESS:
							{
								MsResultNode msresult = malloc(sizeof(MsResultNodeDesc));
								pqGetInt(&(msresult->plan_id), 4, conn);
								msresult->ms_type = MS_SPLIT_SUCCESS;
								conn->result_num++;
								conn->ms_result_list[conn->result_num - 1] = msresult;

								if (conn->inStart + 5 + msgLength == conn->inCursor)
									conn->inStart = conn->inCursor;
								else
									printfPQExpBuffer(&conn->errorMessage,
									  	libpq_gettext("MS: unexpected plan type, plan type was \"%d\"\n"),
									  	type);								
								break;										
							}
							case MS_SPLIT_COMPLETE:
							{
								MsResultNode msresult = malloc(sizeof(MsResultNodeDesc));
								pqGetInt(&(msresult->plan_id), 4, conn);
								msresult->ms_type = MS_SPLIT_COMPLETE;
								conn->result_num++;
								conn->ms_result_list[conn->result_num - 1] = msresult;
						...
						//conn->asyncStatus = PGASYNC_MS_HEARTBEAT;
						continue;
					}
                case 'L':
                    {
                        ...
                        uint64 lts_high = 0;
                        int64 start_ts = 0;
                        pqGetInt64(&(lts_low), conn);
                        pqGetInt64(&(lts_high), conn);
                        pqGetInt64(&(start_ts), conn);
                   		conn->lts_high = lts_high;
                    	conn->lts_low = lts_low;
                        conn->start_ts = (uint64)start_ts;
                        break;
                    }  
				case 'n':		/* No Data */
		...
		else
		{
			...
			/* Trouble --- report it */
			printfPQExpBuffer(&conn->errorMessage,
							  libpq_gettext("message contents do not agree with length in message type \"%c\", msglen %d, instart %d, incursor %d,  \n"),
							  id, msgLength, conn->inStart, conn->inCursor);
			/* build an error result holding the error message */
			pqSaveErrorResult(conn);
			conn->asyncStatus = PGASYNC_READY;
...
/*
 * parseInput subroutine to read a 'M' (M-S heartbeat) message.
 * We use the ssm_statistics to containing these statistics data.
 ...
 */
static int
getMSHeartBeat(PGconn *conn, int msgLength)
{
	const char *errmsg;
	MsResultNode msresult = malloc(sizeof(MsResultNodeDesc));

	SSM_Statistics *ssm_statistics = malloc(sizeof(SSM_Statistics));
	memset(ssm_statistics, 0, sizeof(SSM_Statistics));
	pqGetInt(&(msresult->plan_id), 4, conn);
	pqGetInt((int*)&(ssm_statistics->range_count), 4, conn);
	if (ssm_statistics->range_count > MAXRANGECOUNT)
	{
		errmsg = "range count get error";
		goto ms_error;
	}
	for (int i = 0; i < ssm_statistics->range_count; i++)
	{
		pqGetInt((int*)&(ssm_statistics->range_statistics[i].rangeID), 4, conn);

		pqGetInt((int*)&(ssm_statistics->range_statistics[i].keybytes), 4, conn);
		pqGetInt((int*)&(ssm_statistics->range_statistics[i].keycount), 4, conn);

		pqGetInt((int*)&(ssm_statistics->range_statistics[i].valuebytes), 4, conn);
		pqGetInt((int*)&(ssm_statistics->range_statistics[i].valuecount), 4, conn);

		pqGetInt((int*)&(ssm_statistics->range_statistics[i].readSize), 4, conn);
		pqGetInt((int*)&(ssm_statistics->range_statistics[i].writeSize), 4, conn);
	}

	pqGetInt((int*)&(ssm_statistics->seg_statistics[0].segmentID), 4, conn);

	pqGetInt((int*)&(ssm_statistics->seg_statistics[0].rangeCount), 4, conn);
    pqGetInt((int*)&(ssm_statistics->seg_statistics[0].leaderCount), 4, conn);
    
	pqGetInt64((int64*)&(ssm_statistics->seg_statistics[0].totalSize), conn);
	pqGetInt64((int64*)&(ssm_statistics->seg_statistics[0].userSize), conn);
	pqGetInt64((int64*)&(ssm_statistics->seg_statistics[0].availableSize), conn);

	pqGetInt((int*)&(ssm_statistics->seg_statistics[0].readSize), 4, conn);
	pqGetInt((int*)&(ssm_statistics->seg_statistics[0].writeSize), 4, conn);

	pqGetInt((int*)&(ssm_statistics->seg_statistics[0].location.location_level), 4, conn);
	for (int i = 0; i < ssm_statistics->seg_statistics[0].location.location_level; i++)
	{
		pqGetInt((int*)&(ssm_statistics->seg_statistics[0].location.location_len[i]), 4, conn);
		pqGetnchar((char*)&(ssm_statistics->seg_statistics[0].location.address[i]),
						(size_t)ssm_statistics->seg_statistics[0].location.location_len[i], conn);
	}

	/* RangeDesc section */
	RangeList *rangelist = malloc(sizeof(RangeList));
	pqGetInt((int*)&(rangelist->range_count), 4, conn);

	for (int i = 0; i < rangelist->range_count; i++)
	{
		pqGetInt((int*)&(rangelist->rangeBufferSize[i]), 4, conn);
		pqGetnchar((char*)&(rangelist->buffer[i]),
						(size_t)(rangelist->rangeBufferSize[i]), conn);
	}
	...
	msresult->ms_result = (char*)ssm_statistics;
	msresult->ms_type = MS_HEART_BEAT;
	msresult->rangelist = (char*)rangelist;
	conn->result_num++;
	conn->ms_result_list[conn->result_num - 1] = msresult;
...
/*
 * parseInput subroutine to read a 'M' (M-S heartbeat) message.
 * We use the split_plan to containing these statistics data.
 * Returns: 0 if completed message, EOF if error or not enough data
 * received yet.
 *
 * Note that if we run out of data, we have to suspend and reprocess
 ...
 */
static int
getSplitPreparePlan(PGconn *conn, int msgLength)
{
	const char *errmsg;
	MsResultNode msresult = malloc(sizeof(MsResultNodeDesc));

	SplitPreparePlan sp = malloc(sizeof(SplitPreparePlanDesc));
	memset(sp, 0, sizeof(SplitPreparePlanDesc));
	sp->header.ms_plan_type = MS_SPLIT_PREPARE;

	pqGetInt(&(msresult->plan_id), 4, conn);
	sp->header.plan_id = msresult->plan_id;
	RangeDesc *range = malloc(sizeof(RangeDesc));
	memset(range, 0, sizeof(RangeDesc));
	pqGetInt((int*)&(range->rangeID), 4, conn);
	pqGetInt((int*)&(range->version), 4, conn);
	pqGetInt((int*)&(range->replica_num), 4, conn);
	
	pqGetInt((int*)&(range->startkey.len), 4, conn);
	TupleKey data = malloc(sizeof(range->startkey.len));
	pqGetnchar((char*)(data), range->startkey.len, conn);
	range->startkey.data = data;

	pqGetInt((int*)&(range->endkey.len), 4, conn);
	data = malloc(sizeof(range->endkey.len));
	pqGetnchar((char*)(data), range->endkey.len, conn);
	range->endkey.data = data;

	range->replica = malloc(sizeof(Replica) * range->replica_num);
	if (range->replica_num <= 0)
	{
		errmsg = "range split: replica count get error";
		goto ms_error;
	}
	for (int i = 0; i < range->replica_num; i++)
	{
		Replica replica = malloc(sizeof(ReplicaDesc));

		pqGetInt((int*)&(replica->replicaID), 4, conn);
		pqGetInt((int*)&(replica->LeaderReplicaID), 4, conn);
		pqGetInt((int*)&(replica->segmentID), 4, conn);
		pqGetInt((int*)&(replica->replica_state), 4, conn);
		range->replica[i] = replica;
	}
	sp->split_range = range;

	pqGetInt((int*)&(sp->split_key.len), 4, conn);
	data = malloc(sizeof(sp->split_key.len));

	pqGetnchar((char*)(data), sp->split_key.len, conn);
	sp->split_key.data = data;
	...
	msresult->ms_result = (char*)sp;
	msresult->ms_type = MS_SPLIT_PREPARE;
	conn->result_num++;
	conn->ms_result_list[conn->result_num - 1] = msresult;
	conn->inStart = conn->inCursor;
	return 0;
