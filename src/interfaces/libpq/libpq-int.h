...
/* MsResultNode stores the type of information that the SEG node sends 
 * to the MS node, as well as all the information
 */
typedef struct  msResultNodeDesc
{
	int ms_type;
	char *ms_result;	/* ssm_statistics or rangeplan pointer */
	char *rangelist;
	int plan_id;
}MsResultNodeDesc;

typedef MsResultNodeDesc* MsResultNode;
...
struct pg_result
{
	...
	MsResultNode *ms_result_list;	/* This is where the MsResult node is stored. */
	int result_num;
};
...
typedef enum
{
	...
	PGASYNC_MS_HEARTBEAT		/* Received statistics from the node */
} PGAsyncStatusType;
