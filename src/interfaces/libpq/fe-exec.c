 ...
 */
int
PQsendNCharQuery(PGconn *conn, const char *query, size_t len)
{
	if (!PQsendQueryStart(conn))
		return 0;
	...
	/* construct the outgoing Query message */
	if (pqPutMsgStart('I', false, conn) < 0 ||
		pqPutInt(len, 4, conn) < 0 ||
		pqPutnchar(query, len, conn) < 0 ||
		pqPutMsgEnd(conn) < 0)
	{
		pqHandleSendFailure(conn);
		return 0;
...
static void
RefreshConnResultList(PGconn *conn)
{
	conn->ms_result_list = malloc(sizeof(MsResultNode) * MAX_RESULT_NUM);
	MemSet(conn->ms_result_list, 0, sizeof(MsResultNode) * MAX_RESULT_NUM);
	conn->result_num = 0;
}
...
PQgetResult(PGconn *conn)
{
	...
	switch (conn->asyncStatus)
	{
		...
		case PGASYNC_READY:
			...
			conn->asyncStatus = PGASYNC_BUSY;
			res->ms_result_list = conn->ms_result_list;
			res->result_num = conn->result_num;
			RefreshConnResultList(conn);
			break;
		...
		case PGASYNC_COPY_BOTH:
			...
			break;
		case PGASYNC_MS_HEARTBEAT:
			res = PQmakeEmptyPGresult(conn, PGRES_MS_HEARTBEAT);
			res->ms_result_list = conn->ms_result_list;
			res->result_num = conn->result_num;
			RefreshConnResultList(conn);
			break;
