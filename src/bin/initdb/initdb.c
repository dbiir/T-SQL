...
/* GPDB needs these directories */
	...
	"pg_log",
/* RocksDB needs these directories */
	RocksDBPath,
	RocksDBBackupPath
};
...
void		initialize_rocksdb_instance(const char *dbpath, const char *bkpath);
void		initialize_transaction_instance(const char *dbpath, const char *bkpath);
...
void
initialize_rocksdb_instance(const char *dbpath, const char *bkpath)
{
	...
	//rocksdb_options_optimize_level_style_compaction(options, 64 << 30);
	...
	rocksdb_column_family_handle_t* lts_handel = rocksdb_create_column_family(
		db, options, "lts", &err);
	Assert(!err);
	Assert(lts_handel != NULL);

	rocksdb_column_family_handle_t* cf_handel = rocksdb_create_column_family(
		db, options, RocksDBSystemCatalog, &err);
	Assert(!err);
	Assert(cf_handel != NULL);

	// open Backup Engine that we will use for backing up our database
	rocksdb_backup_engine_t *be = rocksdb_backup_engine_open(options, bkpath, &err);
	Assert(!err);
	Assert(be != NULL);
	...
	// cleanup
    rocksdb_column_family_handle_destroy(lts_handel);
	rocksdb_column_family_handle_destroy(cf_handel);
...
void
initialize_transaction_instance(const char *dbpath, const char *bkpath)
{
	rocksdb_options_t *options = rocksdb_options_create();
	long cpus = sysconf(_SC_NPROCESSORS_ONLN);	// get # of online cores
	rocksdb_options_increase_parallelism(options, (int)(cpus));
    ...
    rocksdb_transactiondb_options_t *dbopt = rocksdb_transactiondb_options_create();
	// open DB
	char *err = NULL;
	rocksdb_transactiondb_t *db = rocksdb_transactiondb_open(options, dbopt, dbpath, &err);
    Assert(!err);
	Assert(db != NULL);

	rocksdb_column_family_handle_t* lts_handel = rocksdb_transactiondb_create_column_family(
		db, options, "lts", &err);
	Assert(!err);
	Assert(lts_handel != NULL);
    
	rocksdb_column_family_handle_t* cf_handel = rocksdb_transactiondb_create_column_family(
		db, options, RocksDBSystemCatalog, &err);
	Assert(!err);
	Assert(cf_handel != NULL);
	...
	// cleanup
    rocksdb_transactiondb_options_destroy(dbopt);
    rocksdb_column_family_handle_destroy(lts_handel);
	rocksdb_column_family_handle_destroy(cf_handel);
	rocksdb_options_destroy(options);
	rocksdb_backup_engine_close(be);
...
initialize_data_directory(void)
{
	...
	printf(_("initializing rocksdb instance ... "));
	fflush(stdout);
	char *dbpath = psprintf("%s/%s", pg_data, RocksDBPath);
	char *bkpath = psprintf("%s/%s", pg_data, RocksDBPath);
	// initialize_rocksdb_instance(dbpath, bkpath);
    initialize_transaction_instance(dbpath, bkpath);
