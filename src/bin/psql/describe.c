		...
		case 'r':
			...
			if(tableinfo.relstorage == 'a')
				...
				printfPQExpBuffer(&title, _("External table \"%s.%s\""),
								  schemaname, relationname);
			else if(tableinfo.relstorage == 'r')
				printfPQExpBuffer(&title, _("RocksDB table \"%s.%s\""),
								  schemaname, relationname);
...
listTables(const char *tabtypes, const char *pattern, bool verbose, bool showSystem)
{
	...
	if (showTables && isGPDB())
	{
		...
		appendPQExpBuffer(&buf, " WHEN 'r' THEN '%s'", gettext_noop("rocksdb"));  /* r is for rocksdb */
	...
	if (showTables || showIndexes || showSeq || (showSystem && showTables))
		appendPQExpBuffer(&buf, "'h', 'a', 'c', 'r',");  /* r is for rocksdb */
	if (showExternal)
