...
typedef struct htf_entry_data
{
	...
	/* The key stored in kv storage. */
	TupleKeySlice key;
	
	/* The range store the kv key*/
	Oid rangeid;
...
extern void htfifo_addtuple(htup_fifo htf, GenericTuple htup, TupleKeySlice key, Oid rangeid);
extern GenericTuple htfifo_gettuple(htup_fifo htf, TupleKeySlice *key, Oid *rangeid);
