...
typedef struct StdRdOptions
{
	...
	char		storage_engine[NAMEDATALEN];	/* storage engine type (currently, only rocksdb takes effect) */
	char		cf_options[CFOPTIONSLEN];			/* column family options for rocksdb */
...
/*
 * RelationIsRocksDB
 * 		True iff relation has rocksdb storage
 */
#define RelationIsRocksDB(relation) \
	((bool)((relation)->rd_rel->relstorage == RELSTORAGE_ROCKSDB))
