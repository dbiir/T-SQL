...
typedef struct RangeTblEntry
{
	...
	Temporal   *temporal;		/* charleyzhao: temporal info */
...
/*
 * The set of columns used to identify a record (tuple) in rocksdb storage.
 * It is currently the columns in primary key, so that a primary key must be 
 * declared for rocksdb storage engine.
 */
typedef struct IdentifiedBy
{
	NodeTag		type;
	int			numkeys;
	List		*keys;
...
typedef struct CreateStmt
{
	...
	IdentifiedBy  *identifiedBy;	/* what columns we use to identify a record in rocksdb */
	Node       *partitionBy;     /* what columns we partition the data by */
	char	    relKind;         /* CDB: force relkind to this */
	char		relStorage;
	Oid 		relRangeID;		/* Rangeid for table initialization */
	int 		replicanum;	
	int32	   *replica_segid;	
