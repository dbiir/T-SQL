...
#define fetchattsize(A) fetch_att_size((A)->attbyval, (A)->attlen)
...
/*
*	[hzy9819]
*	return the length of data on specific type
*	if we meet a pointer(char *), then we return 0 temporally (we will set length correctly before storage it)
*/

#define fetch_att_size(attbyval,attlen) \
( \
	(attbyval) ? \
	( \
		(attlen) == (int) sizeof(Datum) ? \
