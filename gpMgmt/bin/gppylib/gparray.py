...
ROLE_SECONDARY = 's'
ROLE_MIRROR  = 'm'
VALID_ROLES  = [ROLE_PRIMARY, ROLE_SECONDARY, ROLE_MIRROR]
...
class Segment:
    ...
    def isSegmentMaster(self, current_role=False):
        ...
        return self.content == -1 and role == ROLE_PRIMARY

    def isSegmentSecMaster(self, current_role=False):
        role = self.role if current_role else self.preferred_role
        return self.content == -2 and role == ROLE_SECONDARY
...
class GpArray:
    ...
    def __init__(self, segments, segmentsAsLoadedFromDb=None):
        ...
        for segdb in segments:
            ...
            elif segdb.isSegmentSecMaster(True):
                if self.secMaster != None:
                    continue
                self.secMaster = segdb
    ...
    def get_hostlist(self, includeMaster=True):
        ...
        if includeMaster:
            ...
            if self.secMaster is not None:
                hosts.append(self.secMaster.hostname)
        for segPair in self.segmentPairs:
